# -*- coding: utf-8 -*-
"""[P0+19-final fix 5 / 2026-05-16] 一次性"全量跨模块 import 兜底"

给所有 16 个新文件追加 try/except imports：把所有 jarvis_xxx 中可能用到的类
都尝试 import 一次。循环依赖时 try/except 自动忽略（已加载的反向引用走 nerve.py 转发）。

之后 Sir 启动应该不再有 NameError 暴露。
"""
import os
...
def patch(filename):
    ...
    if 'P0+19-final fix 5' in content:
        return f'{filename}: already patched'
    
    # 把 cross imports 追加到文件末尾，避免触发顶部 import 循环
    new = content.rstrip() + '\n\n' + CROSS_IMPORTS
    with open(path, 'w', encoding='utf-8') as f:
        f.write(new)
    return f'{filename}: patched (appended cross imports)'
