...
class ConcernFeedbackJudge:
    ...
    def _build_prompt(self, user_input: str, actives_brief: list) -> str:
        ...
        return (
            ...
            "  命中 → 主脑判 (a 必须有, b/c 按 evidence):\n"
            "    a) ✅ MUST: X 对应 concern (e.g. 面试 → sir_interview_prep_balance):\n"
            "       severity_delta = +0.6 (强升权), has_relevance=true\n"
            "       即使该 concern 当前 severity 几乎 0 / 不在 actives_brief top —\n"
            "       仍要在输出加 entry. cid 找最接近的语义匹配 (面试 → interview / prep)\n"
            "       找不到精确 cid 也要输出 cid='_priority_x_placeholder' + 完整 entry,\n"
            "       让上游 (ConcernsLedger) 知道 X 应升权. 准则 5 言出必行红线.\n"
            ...
            "  ⚠️ Sir 真测痛点 (2026-05-25 21:32): 主脑判了 b (-0.4) 但漏 a (+0.6),\n"
            "    sir_interview_prep_balance severity=1e-16 太低被 LLM 忽略. 修法:\n"
            "    a 是 MUST, 即使 severity 低 / 找不到精确 cid 也输出 entry.\n"
