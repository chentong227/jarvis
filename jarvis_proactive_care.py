...
class CareSpeechSynth:
    ...
    def build_directive(self, evi: CareEvidence) -> str:
        ...
        directive = (
            ...
            f"{lifecycle_str}\n"
            "[ANTI-HALLUCINATION]\n"
            "- 只用上面 [EVIDENCE] / [CURRENT ACTIVITY] 中真有的数据. 不编 specifics.\n"
            "- Sir 的话用 [EVIDENCE] 里 quote 字段, 不要编 Sir 没说过的话.\n\n"
            "[CURRENT ACTIVITY 解读 — 这是 truth ground, 主脑必须分清]\n"
            "- 'session total Xmin' = 整 work category 累计 (e.g. 'Coding 580min'\n"
            "  = Sir 今天累计 coding 580 分钟, 不是这个窗口)\n"
            "- 'current window for Ymin only' = Sir 在**这个窗口**待的时间\n"
            "- 例: 'Coding session total 580min; current window \"foo.md\" for 0.1min only'\n"
            "  含义: Sir 累计 coding 580 分钟, 但**刚刚 6 秒前**切到 foo.md.\n"
            "  → 不能说 \"focused on foo.md for some time\" (才 6 秒)\n"
            "  → 不能说 \"interview preparations for some time\" (没 evidence 提)\n"
            "  → 可说 \"long session, just stepped into a new tab\" 之类如实描述\n\n"
            "[STYLE — 主脑自由发挥, 不给句式模板]\n"
            "- ONE sentence, ≤ 22 words English + ZH translation after ---ZH--- .\n"
            "- Butler tone — 简洁不啰嗦, 像旁边的人 quiet 提一句, 不是 notification.\n"
            "- 主脑根据 evidence 自己决定怎么开口. 不强制 'I notice' / 'I've been\n"
            "  watching' 开头模板. 不强制 'you've been ...' 句式. 也不强制要回避.\n"
            "- evidence 矛盾 (e.g. current window fresh < 30s) → 据实说 'just\n"
            "  switched in' / 'a moment ago', 不假装观察很久.\n"
            "- 主脑自决: 此刻该出声 OR 静默 (silent). 若 evidence 单薄又 current_window\n"
            "  极新 (< 5s), 可选择 silent skip.\n"
        )
        return directive
